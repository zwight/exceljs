...
function getValueType(v) {
  ...
  if (v.cellImageId) return Enums.valueType.Image;
  if (v.formula) {
  ...
  prepare(model, options) {
    ...
    switch (model.type) {
      ...
      case Enums.ValueType.Formula:
        ...
        break;
      case Enums.ValueType.Image:
        model.formula = `DISPIMG("${model.value.cellImageId}", 1)`;
        model.value = `=DISPIMG("${model.value.cellImageId}", 1)`;
        if (model.value.hyperlink) {
          options.hyperlinks.push({
            address: model.value.address,
            target: model.value.hyperlink,
            tooltip: model.value.tooltip,
            mode: model.value.hyperlinkModel,
            display: model.display,
          });
        }
