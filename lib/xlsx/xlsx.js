...
const CellImagesXform = require('./xform/cell-image/cell-images');
...
class XLSX {
  ...
  async addMedia(zip, model) {
    await Promise.all(
      model.media.map(async medium => {
        if (medium.type === 'image' || medium.type === 'cellimage') {
  ...
  addCellImages(zip, model) {
    const cellImageXform = new CellImagesXform();
    const relsXform = new RelationshipsXform();
    const cellImages = model.cellImages.images;
    if (cellImages.length) {
      cellImageXform.prepare(cellImages);
      let xml = cellImageXform.toXml(cellImages);
      zip.append(xml, {name: 'xl/cellimages.xml'});

      xml = relsXform.toXml(model.cellImages.rels);
      zip.append(xml, {name: 'xl/_rels/cellimages.xml.rels'});
    }
  }
  ...
  async addWorkbookRels(zip, model) {
    ...
    const relationships = [
      ...
      {Id: `rId${count++}`, Type: XLSX.RelType.CellImage, Target: 'cellimages.xml'},
    ];
  ...
  prepareModel(model, options) {
    ...
    model.cellImages = model.cellImages || {};
  ...
  async write(stream, options) {
    ...
    await this.addCellImages(zip, model);
